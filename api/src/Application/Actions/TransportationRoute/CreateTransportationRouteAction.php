...
class CreateTransportationRouteAction extends TransportationRouteAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission to create transportation routes
        $scope = Utils::resolveScope($permissions, 'transportation_route', 'create');
        if (empty($scope)) {
        ...
        // Check unrouted purchase tickets
        if ($source_type === 'purchase_ticket') {
            $count_routed = $this->transportationRouteRepository->countUnroutedPurchaseTickets($source_transaction_ticket_ids);
            if ($count_routed) {
                throw new HttpBadRequestException($this->request, "Danh sách phiếu mua đã được phân tuyến vận chuyển");
            }
        }
        ...
        if (empty($data_factory)) {
            throw new HttpNotFoundException($this->request, "Nhà máy không tìm thấy");
        }

        // Create code
        $transportation_route_code = $this->transportationRouteRepository->generateCode();

        // Data Transportation Route
        $data_update = [
            "company_id" => $this->auth_data['company_id'] ?? 0,
            "transportation_route_code" => $transportation_route_code,
        ...
        $transportationRoute = $this->transportationRouteRepository->createTransportationRoute($data_update);
