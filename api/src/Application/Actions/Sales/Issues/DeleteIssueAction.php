...
class DeleteIssueAction extends IssueAction
{
    protected function action(): Response
    {
        ...
        $scope = Utils::resolveScope($permissions, 'sales_issue', 'delete');
        if (empty($scope)) {
        ...
        if (($issueData['status'] ?? 'draft') !== 'draft') {
            throw new HttpBadRequestException($this->request, 'Chỉ xóa phiếu ở trạng thái draft');
        }
        ...
        $ok = $this->salesIssueRepository->deleteIssueWithPermission(
            (int)$issue->getId(),
            $data,
            (int)$this->auth_data['user_id'],
            (string)$scope,
        ...
        if (!$ok) {
            throw new HttpBadRequestException($this->request, 'Không thể xóa phiếu xuất kho');
        }
