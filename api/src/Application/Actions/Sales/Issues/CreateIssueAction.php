...
class CreateIssueAction extends IssueAction
{
    protected function action(): Response
    {
        ...
        $validator->validate('issue_code', $formData['issue_code'] ?? null, 'required|string|max:30');
        ...
        $issue_code = $clean['issue_code'];
        // Check if issue code already exists
        $issue_exists = $this->salesIssueRepository->findIssueOfCode($issue_code);
        if ($issue_exists) {
        ...
        $data = [
            ...
            'issue_date' => $clean['issue_date'] ?? date('Y-m-d H:i:s'),
            'status' => 'draft',
        ...
        $created = $this->salesIssueRepository->createIssue($data, $preparedItems);
        if (!$created) {
            throw new HttpBadRequestException($this->request, 'Không thể tạo phiếu xuất kho');
        }

        $res = ['result' => 'success', 'issue' => $created->jsonSerialize(), 'trace_id' => $trace_id];
