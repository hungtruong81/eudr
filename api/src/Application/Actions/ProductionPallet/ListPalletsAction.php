...
class ListPalletsAction extends ProductionPalletAction
{
    protected function action(): Response
    {
        ...
        $validator->validate('status', $formData['status'] ?? null, 'string|in:closed,all');
        ...
        $result = $this->productionPalletRepository->findAllPallets(
            [
                'page' => $cleanData['page'],
                ...
                'warehouse_id' => $cleanData['warehouse_id'] ?? 0,
                'status' => $cleanData['status'] ?? 'closed',
