...
class UpdatePurchasingOrderItemAction extends PurchasingOrderAction
{
    protected function action(): Response
    {
        ...
        if ($order->getStatus() !== 'draft') {
            throw new HttpBadRequestException($this->request, 'Chỉ được sửa dòng hàng khi phiếu ở trạng thái draft');
        }
        ...
        $item = $this->purchasingOrderRepository->updateOrderItemWithPermission(
            (int)$order->getId(),
            $itemId,
            [
        ...
        if (empty($item)) {
            throw new HttpBadRequestException($this->request, 'Không thể cập nhật dòng hàng hóa');
        }
