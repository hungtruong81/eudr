...
class SetupDryingProductionOrderAction extends ProductionOrderAction
{
    protected function action(): Response
    {
        ...
        $validator->validate('oven_id', $formData['oven_id'] ?? null, 'required|integer|min:1');
        $validator->validate('expected_drying_hours', $formData['expected_drying_hours'] ?? null, 'integer|min:0');
        $validator->validate('expected_final_moisture_percent', $formData['expected_final_moisture_percent'] ?? null, 'numeric|min:0');
        ...
        $cleanData = $validator->sanitize($formData, [
            'oven_id' => 'integer',
            'expected_drying_hours' => 'integer',
            'expected_final_moisture_percent' => 'float',
            'started_at' => 'string',
            'ended_at' => 'string',
            'notes' => 'string',
        ]);
        ...
        try {
            $setup = $this->productionOrderRepository->setupDrying(
                (int)$order->getId(),
                (int)$cleanData['oven_id'],
                isset($cleanData['expected_drying_hours']) ? (int)$cleanData['expected_drying_hours'] : null,
                isset($cleanData['expected_final_moisture_percent']) ? (float)$cleanData['expected_final_moisture_percent'] : null,
                $startedAt,
                $endedAt,
        ...
        $log = [
            ...
            'extra_2' => (string)$cleanData['oven_id'],
        ];
