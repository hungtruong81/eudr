...
class SetupPalletProductionOrderAction extends ProductionOrderAction
{
    protected function action(): Response
    {
        ...
        $validator->validate('planned_pallet_quantity', $formData['planned_pallet_quantity'] ?? null, 'required|integer|min:0');
        ...
        $cleanData = $validator->sanitize($formData, [
            'planned_pallet_quantity' => 'integer',
            'started_at' => 'string',
            'ended_at' => 'string',
            'notes' => 'string',
        ...
        try {
            $setup = $this->productionOrderRepository->setupPallet(
                (int)$order->getId(),
                (int)$cleanData['planned_pallet_quantity'],
                $startedAt,
                $endedAt,
                $cleanData['notes'] ?? null,
                (int)($this->auth_data['company_id'] ?? 0),
                (int)$order->getFactoryId(),
                (int)($cleanData['warehouse_id'] ?? 0),
                (int)$this->auth_data['user_id']
            );
        } catch (\Exception $e) {
        ...
        $log = [
            ...
            'extra_2' => (string)$cleanData['planned_pallet_quantity'],
        ];
