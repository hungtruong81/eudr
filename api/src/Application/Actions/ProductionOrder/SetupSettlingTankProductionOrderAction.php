...
class SetupSettlingTankProductionOrderAction extends ProductionOrderAction
{
    protected function action(): Response
    {
        ...
        $validator->validate('settling_tank_id', $formData['settling_tank_id'] ?? null, 'required|integer|min:1');
        $validator->validate('settling_duration_hours', $formData['settling_duration_hours'] ?? null, 'integer|min:0');
        ...
        try {
            $setup = $this->productionOrderRepository->setupSettlingTank(
                (int)$order->getId(),
                (int)$cleanData['settling_tank_id'],
                isset($cleanData['settling_duration_hours']) ? (int)$cleanData['settling_duration_hours'] : null,
                $startedAt,
                $endedAt,
        ...
        $log = [
            ...
            'action'       => 'setup_settling_tank',
            'user_id'      => (string)$this->auth_data['user_id'],
            'extra_1'      => (string)$order->getId(),
            'extra_2'      => (string)$cleanData['settling_tank_id'],
        ];
