...
class DeleteLandAction extends LandAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission
        $scope = Utils::resolveScope($permissions, 'land', 'delete');

        if (empty($scope)) {
            throw new HttpForbiddenException($this->request, "Thiếu quyền truy cập");
        }

        $land_code = addslashes(trim($this->resolveArg('code')));
        ...
        // Delete land
        $this->landRepository->deleteLandWithPermission(
            $land->getId(),
            (int)$this->auth_data['user_id'],
            (string)$scope,
            $this->auth_data['company_id'] ?? null
