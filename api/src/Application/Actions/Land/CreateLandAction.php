...
class CreateLandAction extends LandAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission to create land
        $scope = Utils::resolveScope($permissions, 'land', 'create');
        if (empty($scope)) {
        ...
        $farmer_user_id = $this->auth_data['user_id'];
        // check duplicate coordinates
        // $check_duplicate = $this->landRepository->checkDuplicateCoordinates($coordinates, $tolerance = 0.000001, 0);
        // if ($check_duplicate) {
        //     throw new LandErrorException("Tọa độ bị trùng lặp, đã có trong hệ thống", 102);
        // }

        // If inspector, only allow create for other user
        if ($user_role === 'inspector') {
            if (!empty($formData['farmer_user_id']) && intval($formData['farmer_user_id']) > 0) {
                $farmer_user_id = intval($formData['farmer_user_id']);
            } else {
                throw new HttpBadRequestException($this->request, "Thiếu trường dữ liệu farmer_user_id");
            }
            // Check connection between inspector and farmer
            ...
            if (empty($data_connection)) {
                throw new HttpBadRequestException($this->request, "Bạn chưa có thông tin kết nối. Vui lòng kết nối và thử lại");
            }
