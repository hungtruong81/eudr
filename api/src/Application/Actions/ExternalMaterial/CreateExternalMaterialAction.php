...
class CreateExternalMaterialAction extends ExternalMaterialAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission
        $scope = $this->userRepository->getCURDPermissionStatus($this->auth_data['user_id'], 'finished_goods_receipt', 'create'); // external_material
        if (empty($scope)) {
        ...
        $validator->validate('plots', $formData['plots'] ?? null, 'required|array');
        ...
        $factory = $this->factoryRepository->findFactoryOfId($factory_id);
        if (empty($factory)) {
            throw new HttpBadRequestException($this->request, "Nhà máy không tồn tại");
        }

        // Validate at least one weight > 0
        if ($latex_weight <= 0 && $scrap_rubber_weight <= 0 && $cup_lump_weight <= 0) {
            throw new HttpBadRequestException($this->request, "Phải có ít nhất một loại nguyên liệu có khối lượng > 0");
        }

        // Validate & process plots — check coordinates for duplicates in eudr_lands
        ...
        foreach ($plots as $index => $plot) {
            ...
            foreach ($coordinates as $coord) {
                if (!isset($coord['lat']) || !isset($coord['lng'])) {
                    throw new HttpBadRequestException($this->request, "Vườn #" . ($index + 1) . " ($plot_name): Tọa độ không hợp lệ, cần có lat và lng");
                }
            }

            // Check duplicate coordinates in existing eudr_lands
            /*
            ...
            */
            ...
            // Create a new Land record with Unknown owner
            ...
            $land_data = [
                ...
                "register_type" => "material_external",
        ...
        // Generate code
        $external_material_code = $this->externalMaterialRepository->generateCode();

        // Create external material record
        $data_insert = [
            "external_material_code" => $external_material_code,
            ...
            "cup_lump_weight" => $cup_lump_weight,
        ...
        $external_material = $this->externalMaterialRepository->createExternalMaterial($data_insert);
