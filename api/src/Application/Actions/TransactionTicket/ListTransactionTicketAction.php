...
class ListTransactionTicketAction extends TransactionTicketAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        $validator->validate('status', $formData['status'] ?? null, 'required|in:all,pending,cancelled,completed');
        ...
        $validator->validate('target_user_id', $formData['target_user_id'] ?? null, 'integer');
        $validator->validate('member_user_id', $formData['member_user_id'] ?? null, 'integer');
        $validator->validate('sales_source', $formData['sales_source'] ?? null, 'in:land,ticket');
        ...
        $target_user_id = $cleanData['target_user_id'] ?? 0;
        $member_user_id = $cleanData['member_user_id'] ?? 0;
        $sales_source = $cleanData['sales_source'] ?? null;
        ...
        $transaction_ticket = $this->transactionTicketRepository->findAll($params);
