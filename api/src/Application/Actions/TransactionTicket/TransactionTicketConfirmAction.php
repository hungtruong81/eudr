...
class TransactionTicketConfirmAction extends TransactionTicketAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        if ($transaction_ticket->getStatus() != 'pending') {
            throw new HttpBadRequestException($this->request, "Phiếu giao dịch không trong trạng thái chờ xác nhận.");
        }
       
        // If the confirmer is the seller (not farmer) and the ticket is pending, check linked purchase tickets
        if(
            ($transaction_ticket->getSellerUserId() == $this->auth_data['user_id']) 
            && ($transaction_ticket->getSellerAccountType() != 'farmer') 
            && ($transaction_ticket->getStatus() == 'pending')
        ) {
            $data_purchase_tickets = $this->transactionTicketRepository->getPurchaseTicketsBySaleTicket($transaction_ticket->getId(), $params = []);
            
            if (empty($data_purchase_tickets['records'])) {
                throw new HttpBadRequestException($this->request, "Vui lòng cập nhật phiếu mua trước khi xác nhận phiếu giao dịch này.");
            }
        }
        ...
        if (!$isSelfTransaction) {
            $data_add_notification = [
                ...
                'title' => "Xác nhận phiếu giao dịch",
                'type' => 'confirm',
                'message' => "Thông báo xác nhận phiếu giao dịch từ " . $this->auth_data['full_name'],
