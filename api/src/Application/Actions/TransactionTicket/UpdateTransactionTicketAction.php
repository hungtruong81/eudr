...
class UpdateTransactionTicketAction extends TransactionTicketAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        //$validator->validate('creator_user_id', $formData['creator_user_id'] ?? null, 'required|integer');
        ...
        //$creator_user_id = $cleanData['creator_user_id'];
        ...
        if ($isSelfTransaction) {
            ...
            // Giao dịch giữa 2 user khác nhau: kiểm tra quyền buyer
        ...
        $transaction_ticket = $this->transactionTicketRepository->updateTransactionTicket($transaction_ticket->getId(), $data_update);
        ...
        if (!$isSelfTransaction) {
            $data_add_notification = [
                ...
                'title' => "Phiếu giao dịch đã được cập nhật",
                'type' => 'update',
                'message' => "Phiếu giao dịch đã được cập nhật bởi " . $this->auth_data['full_name'],
