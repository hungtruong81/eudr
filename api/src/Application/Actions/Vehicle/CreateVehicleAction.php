...
class CreateVehicleAction extends VehicleAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission to create vehicles
        $scope = Utils::resolveScope($permissions, 'vehicle', 'create');
        if (empty($scope)) {
        ...
        $vehicle_existing = $this->vehicleRepository->findVehicleOfLicensePlate($license_plate);
        if (!empty($vehicle_existing)) {
            throw new HttpBadRequestException($this->request, "Biển số xe đã tồn tại");
        }

        // Generate vehicle code
        $vehicle_code = $this->vehicleRepository->generateCode();
        ...
        $vehicle = $this->vehicleRepository->createVehicle($data_update);
