...
class DeleteFinishedGoodsReceiptAction extends FinishedGoodsReceiptAction
{
    /**
     * {@inheritdoc}
     */
    protected function action(): Response
    {
        ...
        // Check permission to delete finished goods receipts
        $scope = Utils::resolveScope($permissions, 'finished_goods_receipt', 'delete');
        if (empty($scope)) {
            throw new HttpForbiddenException($this->request, "Thiếu quyền truy cập");
        }

        $finished_goods_receipt_code = addslashes(trim($this->resolveArg('code')));
        ...
        // Delete product type
        $this->finishedGoodsReceiptRepository->deleteFinishedGoodsReceipt($finished_goods_receipt->getId(), $this->auth_data['user_id']);
