...
import { getProductChannels, deleteProductChannel } from "../actions";
import { IProductionChannel } from "../types";
import CustomTable, { CustomColumnTypeTable } from "@/components/custom-table";
import { TooltipButton } from "@/components/tooltip-button";
import { ProductChannelFilter } from "./product-channel-filter";
import { ProductChannelSheet } from "./product-channel-sheet";
...
export const ProductChannel = () => {
  ...
  const [selectedRecord, setSelectedRecord] =
    useState<IProductionChannel | null>(null);
  const [filterForm] = Form.useForm();
  const queryClient = useQueryClient();

  const { data, isLoading } = useQuery({
    queryKey: ["product-channels", params],
    queryFn: () => getProductChannels(params),
  });

  const deleteMutation = useMutation({
    mutationFn: deleteProductChannel,
    onSuccess: () => {
  ...
  const handleEdit = (record: IProductionChannel) => {
    setSelectedRecord(record);
    setOpen(true);
  };

  const handleDelete = (record: IProductionChannel) => {
    deleteMutation.mutate(record.channel_code);
  };
  ...
  const columns: CustomColumnTypeTable<IProductionChannel>[] = [
    {
      title: t("name"),
          ...
          {(factory.full || factory.delete) && (
            <ConfirmTooltipButton
              ...
              loading={
                ...
                deleteMutation.variables === record.channel_code
              }
            />
          )}
        </Space>
  ...
  return (
    <Space direction="vertical" style={{ width: "100%" }} size="middle">
      ...
      <CustomTable<IProductionChannel>
        rowKey="channel_id"
        columns={columns}
        dataSource={data?.data?.records || []}
        loading={isLoading}
        tableId="product-channel-table"
        pagination={{
